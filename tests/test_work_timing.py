...
def test_webhook_notifier_success_records_delivery_metadata(app, monkeypatch):
    with app.app_context():
        app.config["WORK_TIMING_ALERT_MODE"] = "webhook"
        app.config["WORK_TIMING_WEBHOOK_URL"] = "https://n8n.example.com/webhook/work-timing-alert"
        app.config["WORK_TIMING_WEBHOOK_TOKEN"] = "shared-secret"
        app.config["WORK_TIMING_WEBHOOK_TIMEOUT_SECONDS"] = 10
        app.config["WORK_TIMING_GIRI_CONTACT"] = "+919999999999"

        order = Order(order_id="ORDER-WT-HOOK-OK", customer_name="Webhook Customer")
        bootstrap_order_rows(order)
        db.session.add(order)
        db.session.flush()
        base = datetime.utcnow() - timedelta(hours=1)
        ...
        def fake_post_json_webhook(*, url, headers, payload, timeout_seconds):
            assert url == app.config["WORK_TIMING_WEBHOOK_URL"]
            assert headers.get("X-Internal-Token") == app.config["WORK_TIMING_WEBHOOK_TOKEN"]
            assert payload["target"] == "GIRI"
            assert payload["contact"] == app.config["WORK_TIMING_GIRI_CONTACT"]
            assert payload["alert_id"].startswith(f"wt-{entry_id}-GIRI-")
        ...
        monkeypatch.setattr("app.work_timing.services._post_json_webhook", fake_post_json_webhook)

        summary = run_work_timing_overdue_check(now=base + timedelta(minutes=3))
        assert summary["giri_sent"] == 1
        assert summary["md_sent"] == 0

        event = WorkTimingAlertEvent.query.filter_by(entry_id=entry_id).first()
        assert event is not None
        assert event.delivery_mode == "whatsapp_template"
        assert event.delivery_result == "sent"
        assert event.provider_message_id == "wamid.HBgM123"


def test_webhook_notifier_failure_records_failed_result_without_crash(app, monkeypatch):
    with app.app_context():
        app.config["WORK_TIMING_ALERT_MODE"] = "webhook"
        app.config["WORK_TIMING_WEBHOOK_URL"] = "https://n8n.example.com/webhook/work-timing-alert"
        app.config["WORK_TIMING_WEBHOOK_TOKEN"] = "shared-secret"
        ...
        order = Order(order_id="ORDER-WT-HOOK-ERR", customer_name="Webhook Error Customer")
        bootstrap_order_rows(order)
        db.session.add(order)
        db.session.flush()
        base = datetime.utcnow() - timedelta(hours=1)
        ...
        assert event.delivery_result.startswith("failed_")
